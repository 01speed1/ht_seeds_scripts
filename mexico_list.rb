mexico_list = {"Aguascalientes":[
  ...
  "Quintana Roo":[
  "Cozumel","Felipe Carrillo Puerto","Isla Mujeres","Othón P. Blanco","Benito Juárez","José María Morelos","Lázaro Cárdenas","Solidaridad","Tulum","Bacalar","Puerto Morelos"
  ],
  "San Luis Potosí":[
  ...
  #count
  mexico_list.map { |state, cities_list| cities_list.count }.reduce(0) { |acc, value| acc + value }

  mex = Country.find_or_create_by(description: "México")

  mexico_list.map do |state, cities_list|
      created_state = State.find_or_create_by(description: state.to_s, country: mex)
      cities_list.map { |city| City.find_or_create_by(description: city.to_s, state: created_state) }
  end

  ####################################################################################################

  #to delete if somethings wrong
  State.where(country: mex).map { |state| City.where( state: state ).map { |city| city.destroy }  }